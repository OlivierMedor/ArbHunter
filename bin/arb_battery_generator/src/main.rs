...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1. Success Case (V2) - Aerodrome Stable DAI/USDC/DAI
    ...
    cases.push(HistoricalCase {
        ...
        notes: "Aerodrome Stable circular DAI/USDC/DAI.".into(),
        fork_block_number: block,
        source_tx_hash: None,
        root_asset: TokenAddress(token0.to_string()),
        route_family: "ReserveBased_Circular".into(),
        pool_ids: vec![p_dai_usdc.to_string(), p_dai_usdc.to_string()],
        pool_kinds: vec![PoolKind::ReserveBased, PoolKind::ReserveBased],
        path_tokens: vec![TokenAddress(token0.to_string()), TokenAddress(token1.to_string()), TokenAddress(token0.to_string())],
        leg_directions: vec![true, false],
    ...
    // 3. V3 success case (Circular) - USDC/WETH/USDC
    ...
    cases.push(HistoricalCase {
        case_id: "case_3_v3_success".into(),
        notes: "Uniswap V3 circular USDC/WETH/USDC.".into(),
        fork_block_number: block,
        source_tx_hash: None,
        root_asset: TokenAddress(token0_v3.to_string()),
        route_family: "ConcentratedLiquidity_Circular".into(),
        pool_ids: vec![p_v3_usdc_weth.to_string(), p_v3_usdc_weth.to_string()],
        pool_kinds: vec![PoolKind::ConcentratedLiquidity, PoolKind::ConcentratedLiquidity],
        path_tokens: vec![TokenAddress(token0_v3.to_string()), TokenAddress(token1_v3.to_string()), TokenAddress(token0_v3.to_string())],
        leg_directions: vec![true, false],
        amount_in: U256::from(100_000_000u64), // 0.1 USDC
        expected_outcome: "success".into(),
        guard_overrides: None,
        seed_data: None,
    });
